...
func (ss *ScaleSet) ensureHostsInPool(ctx context.Context, service *v1.Service, nodes []*v1.Node, backendPoolID string, vmSetNameOfLB string) error {
	...
	for meta, update := range nodeUpdates {
		...
		hostUpdates = append(hostUpdates, func() error {
			...
			errChan := UpdateVMSSVMsInBatch(ctx, update, batchSize, func(ctx context.Context, instanceID string, vm *armcompute.VirtualMachineScaleSetVM) (*runtime.Poller[armcompute.VirtualMachineScaleSetVMsClientUpdateResponse], error) {
...
func UpdateVMSSVMsInBatch(ctx context.Context, update map[string]armcompute.VirtualMachineScaleSetVM, batchSize int, patchVMFn func(ctx context.Context, instanceID string, vm *armcompute.VirtualMachineScaleSetVM) (*runtime.Poller[armcompute.VirtualMachineScaleSetVMsClientUpdateResponse], error)) <-chan error {
	logger := klog.FromContext(ctx)
	if patchVMFn == nil {
		logger.Error(fmt.Errorf("patchVMFn is nil"), "Failed to update VMs for VMSS with new backendPoolID")
		return nil
	}
